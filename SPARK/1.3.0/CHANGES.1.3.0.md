...
| [SPARK-4700](https://issues.apache.org/jira/browse/SPARK-4700) | Add Http support to Spark Thrift server |  Major | SQL | Judy Nash |  |
...
| [SPARK-5999](https://issues.apache.org/jira/browse/SPARK-5999) | Remove duplicate Literal matching block |  Minor | SQL | Liang-Chi Hsieh |  |
...
| [SPARK-5862](https://issues.apache.org/jira/browse/SPARK-5862) | Only transformUp the given plan once in HiveMetastoreCatalog |  Minor | SQL | Liang-Chi Hsieh |  |
...
| [SPARK-5789](https://issues.apache.org/jira/browse/SPARK-5789) | Throw a better error message if JsonRDD.parseJson encounters unrecoverable parsing errors. |  Major | SQL | Yin Huai |  |
...
| [SPARK-5780](https://issues.apache.org/jira/browse/SPARK-5780) | The loggings of Python unittests are noisy and scaring in |  Major | PySpark | Davies Liu |  |
...
| [SPARK-5758](https://issues.apache.org/jira/browse/SPARK-5758) | Use LongType as the default type for integers in JSON schema inference. |  Major | SQL | Yin Huai |  |
...
| [SPARK-3928](https://issues.apache.org/jira/browse/SPARK-3928) | Support wildcard matches on Parquet files |  Minor | Spark Core, SQL | Nicholas Chammas |  |
...
| [SPARK-6134](https://issues.apache.org/jira/browse/SPARK-6134) | Fix wrong datatype for casting FloatType and default LongType value in defaultPrimitive |  Major | SQL | Liang-Chi Hsieh |  |
...
| [SPARK-6082](https://issues.apache.org/jira/browse/SPARK-6082) | SparkSQL should fail gracefully when input data format doesn't match expectations |  Major | SQL | Kay Ousterhout |  |
...
| [SPARK-6073](https://issues.apache.org/jira/browse/SPARK-6073) | Need to refresh metastore cache after append data in CreateMetastoreDataSourceAsSelect |  Blocker | SQL | Yin Huai |  |
...
| [SPARK-6052](https://issues.apache.org/jira/browse/SPARK-6052) | In JSON schema inference, we should always set containsNull of an ArrayType to true |  Blocker | SQL | Yin Huai |  |
...
| [SPARK-6040](https://issues.apache.org/jira/browse/SPARK-6040) | Fix the percent bug in tablesample |  Major | SQL | Yadong Qi |  |
...
| [SPARK-6023](https://issues.apache.org/jira/browse/SPARK-6023) | ParquetConversions fails to replace the destination MetastoreRelation of an InsertIntoTable node to ParquetRelation2 |  Blocker | SQL | Yin Huai |  |
...
| [SPARK-6016](https://issues.apache.org/jira/browse/SPARK-6016) | Cannot read the parquet table after overwriting the existing table when spark.sql.parquet.cacheMetadata=true |  Blocker | SQL | Yin Huai |  |
...
| [SPARK-5984](https://issues.apache.org/jira/browse/SPARK-5984) | TimSort broken |  Minor | Spark Core | Reynold Xin |  |
...
| [SPARK-5967](https://issues.apache.org/jira/browse/SPARK-5967) | JobProgressListener.stageIdToActiveJobIds never cleared |  Blocker | Web UI | Tathagata Das | Tathagata Das |
...
| [SPARK-5950](https://issues.apache.org/jira/browse/SPARK-5950) | Insert array into a metastore table saved as parquet should work when using datasource api |  Blocker | SQL | Liang-Chi Hsieh |  |
...
| [SPARK-5926](https://issues.apache.org/jira/browse/SPARK-5926) | [SQL] DataFrame.explain() return false result for DDL command |  Major | SQL | Yanbo Liang |  |
...
| [SPARK-5875](https://issues.apache.org/jira/browse/SPARK-5875) | logical.Project should not be resolved if it contains aggregates or generators |  Blocker | SQL | Yin Huai |  |
...
| [SPARK-5852](https://issues.apache.org/jira/browse/SPARK-5852) | Fail to convert a newly created empty metastore parquet table to a data source parquet table. |  Blocker | SQL | Yin Huai |  |
...
| [SPARK-5840](https://issues.apache.org/jira/browse/SPARK-5840) | HiveContext cannot be serialized due to tuple extraction |  Major | SQL | Reynold Xin |  |
...
| [SPARK-5833](https://issues.apache.org/jira/browse/SPARK-5833) | Adds REFRESH TABLE command to refresh external data sources tables |  Major | SQL | Cheng Lian |  |
...
| [SPARK-5825](https://issues.apache.org/jira/browse/SPARK-5825) | Failure stopping Services while command line argument is too long |  Blocker | Deploy | Cheng Hao | Cheng Hao |
| [SPARK-5824](https://issues.apache.org/jira/browse/SPARK-5824) | CTAS should set null format in hive-0.13.1 |  Major | SQL | Adrian Wang |  |
...
| [SPARK-5741](https://issues.apache.org/jira/browse/SPARK-5741) | Support the path contains comma in HiveContext |  Major | SQL | Yadong Qi |  |
...
| [SPARK-5361](https://issues.apache.org/jira/browse/SPARK-5361) | Multiple Java RDD \<-\> Python RDD conversions not working correctly |  Major | PySpark | Winston Chen |  |
...
| [SPARK-5153](https://issues.apache.org/jira/browse/SPARK-5153) | Flaky test: o.a.s.streaming.kafka.ReliableKafkaStreamSuite |  Critical | Streaming | Nan Zhu |  |
...
| [SPARK-4986](https://issues.apache.org/jira/browse/SPARK-4986) | Graceful shutdown for Spark Streaming does not work in Standalone cluster mode |  Blocker | Streaming | Jesper Lundgren |  |
...
| [SPARK-4928](https://issues.apache.org/jira/browse/SPARK-4928) | Operator "\>,\<,\>=,\<=" with decimal between different precision report error |  Major | SQL | guowei |  |
...
| [SPARK-4913](https://issues.apache.org/jira/browse/SPARK-4913) | Fix incorrect event log path |  Major | . | Liang-Chi Hsieh | Liang-Chi Hsieh |
| [SPARK-4910](https://issues.apache.org/jira/browse/SPARK-4910) | Build failed with Hadoop 1.x profile: "isFile is not a member of org.apache.hadoop.fs.FileStatus" |  Minor | . | Tomas Hudik | Sean Owen |
...
| [SPARK-4828](https://issues.apache.org/jira/browse/SPARK-4828) | sum and avg over empty table should return null |  Minor | SQL | Adrian Wang |  |
...
| [SPARK-4786](https://issues.apache.org/jira/browse/SPARK-4786) | Parquet filter pushdown for BYTE and SHORT types |  Major | SQL | Cheng Lian |  |
...
| [SPARK-4756](https://issues.apache.org/jira/browse/SPARK-4756) | sessionToActivePool  grow infinitely, even as sessions expire |  Major | SQL | guowei |  |
...
| [SPARK-4720](https://issues.apache.org/jira/browse/SPARK-4720) | Remainder should also return null if the divider is 0. |  Major | SQL | Takuya Ueshin |  |
...
| [SPARK-4713](https://issues.apache.org/jira/browse/SPARK-4713) | SchemaRDD.unpersist() should not raise exception if it is not cached. |  Minor | SQL | Cheng Hao |  |
| [SPARK-4707](https://issues.apache.org/jira/browse/SPARK-4707) | Reliable Kafka Receiver can lose data if the block generator fails to store data |  Critical | Streaming | Hari Shreedharan |  |
...
| [SPARK-4694](https://issues.apache.org/jira/browse/SPARK-4694) | Long-run user thread(such as HiveThriftServer2) causes the 'process leak' in yarn-client mode |  Major | YARN | SaintBacchus |  |
| [SPARK-4693](https://issues.apache.org/jira/browse/SPARK-4693) | PruningPredicates may be wrong if predicates contains an empty AttributeSet() references |  Minor | SQL | YanTang Zhai |  |
...
| [SPARK-4673](https://issues.apache.org/jira/browse/SPARK-4673) | Optimizing limit using coalesce |  Major | SQL | Fei Wang |  |
...
| [SPARK-4662](https://issues.apache.org/jira/browse/SPARK-4662) | Whitelist more Hive unittest |  Major | SQL | Cheng Hao |  |
...
| [SPARK-4623](https://issues.apache.org/jira/browse/SPARK-4623) | Add the some error infomation if using spark-sql in yarn-cluster mode |  Major | Deploy | SaintBacchus |  |
...
| [SPARK-4269](https://issues.apache.org/jira/browse/SPARK-4269) | Make wait time in BroadcastHashJoin configurable |  Major | SQL | Jacky Li |  |
...
| [SPARK-1670](https://issues.apache.org/jira/browse/SPARK-1670) | PySpark Fails to Create SparkContext Due To Debugging Options in conf/java-opts |  Major | PySpark | Pat McDonough |  |
...
| [SPARK-5050](https://issues.apache.org/jira/browse/SPARK-5050) | Add unit test for sqdist |  Minor | . | Liang-Chi Hsieh | Liang-Chi Hsieh |
...
| [SPARK-4798](https://issues.apache.org/jira/browse/SPARK-4798) | Refactor Parquet test suites |  Major | SQL | Cheng Lian |  |
| [SPARK-4631](https://issues.apache.org/jira/browse/SPARK-4631) | Add real unit test for MQTT |  Critical | Streaming | Tathagata Das |  |
| [SPARK-4527](https://issues.apache.org/jira/browse/SPARK-4527) | Add BroadcastNestedLoopJoin operator selection testsuite |  Minor | SQL | XiaoJing wang |  |
...
| [SPARK-5935](https://issues.apache.org/jira/browse/SPARK-5935) | Accept MapType in the schema provided to a JSON dataset. |  Major | SQL | Yin Huai |  |
...
| [SPARK-5465](https://issues.apache.org/jira/browse/SPARK-5465) | Data source version of Parquet doesn't push down And filters properly |  Blocker | SQL | Cheng Lian |  |
...
| [SPARK-5310](https://issues.apache.org/jira/browse/SPARK-5310) | Update SQL programming guide for 1.3 |  Critical | SQL | Reynold Xin |  |
...
| [SPARK-5235](https://issues.apache.org/jira/browse/SPARK-5235) | Determine serializability of SQLContext |  Major | . | Alex Baretta | Reynold Xin |
...
| [SPARK-4843](https://issues.apache.org/jira/browse/SPARK-4843) | Squash ExecutorRunnable and ExecutorRunnableUtil hierarchy in yarn module |  Major | . | Kostas Sakellis | Kostas Sakellis |
...
| [SPARK-5944](https://issues.apache.org/jira/browse/SPARK-5944) | Python release docs say SNAPSHOT + Author is missing |  Minor | Documentation | Nicholas Chammas |  |
...
| [SPARK-4361](https://issues.apache.org/jira/browse/SPARK-4361) | SparkContext HadoopRDD is not clear about how to use a Hadoop Configuration |  Minor | Documentation | Shixiong Zhu |  |
...
| [SPARK-6163](https://issues.apache.org/jira/browse/SPARK-6163) | jsonFile should be backed by the data source API |  Critical | SQL | Yin Huai |  |
