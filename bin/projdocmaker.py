...
class JiraIter:
  ...
  def __init__(self, versions,project):
    ...
    count=100
    while (at < end):
      params = urllib.urlencode({'jql': "project in ("+project+") and fixVersion in ('"+"' , '".join([str(v).replace("-SNAPSHOT","") for v in versions])+"') and resolution = Fixed", 'startAt':at, 'maxResults':count})
...
def main():
  parser = OptionParser(usage="usage: %prog --project PROJECT --version VERSION [--version VERSION2 ...]",
		epilog=
  ...
  jlist = JiraIter(versions,options.project)
  version = maxVersion
  ...
  relhead = '# %(key)s %(ver)s Release Notes\n\n' \
